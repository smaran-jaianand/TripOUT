...
/* ────────────────────────────────────────────────────────────
   Theme hook — persists to localStorage, respects system pref
   ──────────────────────────────────────────────────────────── */
function useTheme() {
...
function Sidebar({ activeTab, onTabChange }) {
  return (
    <nav className="fixed left-0 top-0 h-screen w-64 border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex flex-col py-6 px-4 z-50">
      {/* Brand */}
      <div className="mb-8 px-2 flex items-center gap-3">
        <div className="w-8 h-8 rounded-full bg-primary flex items-center justify-center text-on-primary shrink-0">
          <Icon name="spa" filled className="text-sm" />
        </div>
        <div className="overflow-hidden">
          <h1 className="font-headline text-xl font-bold text-zinc-900 dark:text-zinc-50 tracking-tight leading-none truncate">
            Verdant Planner
          </h1>
          ...
          return (
            <li key={item.id}>
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded text-left transition-colors duration-200 ${
                  active
                    ? "text-primary dark:text-[#b1deb6] font-semibold bg-zinc-100 dark:bg-zinc-900 border-r-2 border-primary"
          ...
          {FOOTER_ITEMS.map((item) => (
            <li key={item.id}>
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2 rounded text-left text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-colors duration-200"
              >
...
function TopBar({ title, theme, onToggleTheme }) {
  ...
  return (
    <header className="fixed top-0 right-0 left-64 h-16 border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md flex justify-between items-center px-8 z-40">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative hidden md:block w-64">
          ...
          />
          <input
            className="w-full pl-9 pr-4 py-1.5 bg-zinc-100 dark:bg-zinc-900 border-none rounded-full text-sm font-label focus:ring-1 focus:ring-primary outline-none text-zinc-900 dark:text-zinc-50 placeholder:text-zinc-400"
...
function WorkspaceView() {
  ...
  return (
    <div className="flex-1 p-8 overflow-y-auto flex flex-col max-w-4xl mx-auto w-full view-enter">
                ...
                {msg.plans && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {msg.plans.map((plan) => (
                      <div
                        key={plan.id}
                        className={`bg-surface-container-lowest dark:bg-zinc-900 rounded-xl border p-5 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group card-enter ${
                          selectedPlan === plan.id
                            ? "border-primary ring-2 ring-primary/20"
                            : "border-outline-variant dark:border-zinc-800"
                        }`}
                      >
                        <div className={`absolute top-0 left-0 w-full h-1 bg-${plan.accent}`} />
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <span
                              className={`inline-block px-2.5 py-1 bg-${plan.accent}/10 text-${plan.accent} font-label font-bold text-xs rounded uppercase tracking-wider mb-2`}
                            >
                              {plan.label}
                            </span>
                            <h3 className="font-headline text-xl font-bold text-on-surface dark:text-zinc-50">
                              {plan.name}
                            </h3>
                          </div>
                          <Icon name={plan.icon} className={`text-${plan.accent}`} />
                        </div>
                        <p className="font-body text-sm text-on-surface-variant dark:text-zinc-400 mb-4 leading-relaxed">
                          {plan.description}
                        </p>
                        <ul className="space-y-2 mb-6 font-label text-sm text-on-surface dark:text-zinc-300">
                        ...
                        <button
                          ...
                          className={`w-full py-2 rounded font-label text-sm font-semibold transition-colors ${
                            selectedPlan === plan.id
      ...
      {/* Input area */}
      <div className="fixed bottom-0 left-64 right-0 p-6 bg-gradient-to-t from-background dark:from-zinc-950 via-background/90 dark:via-zinc-950/90 to-transparent">
        ...
        <p className="text-center mt-2 font-label text-[10px] text-on-surface-variant/60 dark:text-zinc-500 uppercase tracking-widest">
          Verdant Planner AI may produce inaccurate information.
        </p>
      </div>
    </div>
  );
...
function VendorsView() {
  ...
  const filtered = useMemo(() => {
    if (activeFilter === "All Categories") return VENDORS;
...
export default function App() {
  ...
  return (
    <div className="bg-background dark:bg-zinc-950 text-on-background dark:text-zinc-300 font-body antialiased min-h-screen flex">
